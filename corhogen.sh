...
run_update_sources() {
    ...
    # Create directories
    # TODO: Check if deposit is present
    local deposit="$(find "/var/media" -maxdepth 1 -type d | sort -r | head -1)"
    mkdir -p "$deposit/Animes"
    mkdir -p "$deposit/Movies"
    mkdir -p "$deposit/Series"
...
run_update_transmission() {
    ...
    # TODO: Check if deposit is present
    local deposit="$(find "/var/media" -maxdepth 1 -type d | sort -r | head -1)"
    mkdir -p "$deposit/Shared"
    mkdir -p "$deposit/Shared/Incomplete"
    ...
    local deposit="$(find /var/media -maxdepth 1 -type d | sort -r | head -1)"
    /opt/bin/jq ".\"download-dir\" = \"$deposit/Shared\"" "$configs" | /opt/bin/sponge "$configs"
    /opt/bin/jq ".\"incomplete-dir\" = \"$deposit/Shared/Incomplete\"" "$configs" | /opt/bin/sponge "$configs"
